...
- **Fonts:** Newsreader (serif, headings) and Inter (UI/body), plus Comfortaa (500) only for the `Soyun` brand name and the `Work` / `About` / `Resume` nav, loaded from Google Fonts in each page's `<head>`. Keep that `<link>` identical across pages.
...
- **Page width is shared:** every page (landing, About, case studies) uses the same `--gutter` (12vw on desktop >= 960px, max 260px; 10.5vw below that) and `--gap` (13px) from `:root`, so the header, content and footer line up across pages. Don't give a single page its own gutter. About-specific styles go in the "About page" section of `styles.css`. Description text (the About intro paragraphs, the case-study .cs-hero p.sub) has no max-width, so its box always spans the full content width like everything else; don't re-add a ch cap to either without asking.

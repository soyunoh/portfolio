...
- **Mouse pointer:** the owner's own 16px pixel design, used as-is (they asked for their PNG directly, no redraws): `images/cursor-sun.png` is their attached file byte for byte (a small ring with four short rays, ink `#1C1A17`, transparent centre, no halo). Clickable areas (`a[href]`, `button`) use `images/cursor-sun-link.png`: the same mark with the ring filled solid, instead of the pointing hand. Hotspot is the centre (8 8); cursor images must stay <= 128px. There are deliberately no @2x/@3x or SVG versions; on retina screens the browser scales the 16px file. Keep comparison files OUTSIDE the project so nothing gets overwritten.

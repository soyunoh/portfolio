...
- **Fonts:** Newsreader (serif, headings) and Inter (UI/body), plus Comfortaa (500) only for the `Soyun` brand name and the `Work` / `About` / `Resume` nav, loaded from Google Fonts in each page's `<head>`. The landing headline (`.hero h1`) is set in Inter at the same size as the About intro paragraphs (clamp(16px, 1.35vw, 19px), regular weight) on purpose, to match; don't switch it back to Comfortaa/38px without checking. Keep that `<link>` identical across pages.
...
- **Page width is shared:** every page (landing, About, case studies) uses the same `--gutter` (12vw on desktop >= 960px, max 260px; 10.5vw below that) and `--gap` (13px) from `:root`, so the header, content and footer line up across pages. Don't give a single page its own gutter. About-specific styles go in the "About page" section of `styles.css`. Description text (the landing hero h1, the About intro paragraphs, the case-study .cs-hero p.sub) has no max-width, so its box always spans the full content width like everything else; don't re-add a ch cap to any of them without asking. The hero h1 also has no `text-wrap: balance` on purpose: balance shortens lines for visual evenness even when the box has room, which left large gaps before the Resume nav link; plain wrapping fills each line greedily instead, so only the true final line is short. Don't re-add balance there without checking this.

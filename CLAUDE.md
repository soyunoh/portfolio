...
- **Mouse pointer:** `html { cursor: url(images/cursor-sun.svg) 4 4, auto }` in `styles.css`. `cursor-sun.svg` is a 9px version of the owner's `logo-1b` sun (strokes thickened so they survive at that size, white halo so it shows on dark photos). Clickable elements (`a[href]`, `button`) use `images/cursor-sun-link.svg`: the same sun with its centre circle filled in the line colour (`#1C1A17`) instead of the pointing hand. Both fall back to the normal cursors. Cursor images must stay <= 128px.

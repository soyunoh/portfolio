...
- **Mouse pointer:** the owner's `logo-1b` sun redrawn as a 20px cursor: 8 rays 45deg apart (like the header logo; the owner said at most 9 rays), a small dot for the centre, a white halo so it shows on dark photos. `images/cursor-sun.svg` + `cursor-sun{,@2x,@3x}.png` for normal areas (small dot); `cursor-sun-link.*` for `a[href]`/`button` (larger filled dot, no pointing hand). CSS gives the SVG first and `-webkit-image-set` PNGs after it, so retina screens get a sharp image and other browsers fall back to the SVG or the default cursor. Hotspot is the centre (10 10). Cursor images must stay <= 128px. If you change the design, regenerate all PNGs from the SVGs (draw the SVG onto a canvas at 1x/2x/3x).
